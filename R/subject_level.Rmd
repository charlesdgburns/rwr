...
Below is the code used data simulation and visualisation for 'Data-driven estimates of the reproducibility of univariate BWAS are biased.'
...
### Statistical errors are biased under the null (Random data simulations)
...
```{r Figure 2: Estimated statistical errors and reproducibility of random noise, include = FALSE}
### Graphs ###
...
ggsave('figures/Figure1WithoutReplacement.png', 
       (figure1),bg='transparent',  width = 32, height = 16, dpi = 900, units = "cm", device='png')
```

```{r Extended Data: Expected statistical errors and reproducibility of random noise, include=FALSE}
...
ExtData<-ggarrange(NTtype2, NTtypeM, NTtypeS, NTpower, NTrepli, NTtype1,  nrow=2, ncol=3, labels=c('a','b','c','d', 'e', 'f'))

ggsave('figures/ExtData.png', 
       (ExtData),bg='transparent',  width = 32, height = 16, dpi = 900, units = "cm", device='png')

```

### Explaining the bias - "relative sampling variability"
...
os_corrsHist  <- NScorrs %>% as.tibble() %>%
  ...
  labs(subtitle="Large Sample, 1,225 correlations (Pearson)", x="brain-behaviour correlation", y="Frequency")+
...
os_pvalHist  <-NSpvals %>% as.tibble() %>%
  ...
  labs(subtitle="Large sample, 1,225 correlations (Pearson)", x="brain-behaviour P value", y="Frequency")+
  theme_classic()
...
Figure3 <- ggarrange(os_corrsHist, os_pvalHist,  rs_corrsHist,  rs_pvalHist, nrow=2, ncol=2, heights=c(4,4), labels=c('a','b','c','d'))
...
ggsave('figures/Figure3e.png', (NSedgeIterHist), bg='transparent', width=7, height=7, dpi=900, units='cm', device='png')
...
The figure below is not included in any text, but neatly summarises bias in statistical power under the null.

```{r Figure X: Correcting biases under null, include=FALSE}

figureX <- ggarrange(NTpower+labs(subtitle='Generated samples (Ground Truth)', x='Generated sample size (log scale)'), 
...
ggsave('figures/FigureX.png', figureX, device='png', width=20, height=16, units='cm')

```

### Subject level true effects
...
```{r Supplementary figure 4: Visualising copula samples, include=FALSE}
...
SupFig4<-ggarrange(ggarrange(cs1_scatter, cs1_edgeHist, labels=c('a','b')), cs1_corrsHist, nrow=2, labels=c('','c')) #Visualising effect sizes in scatter plots

ggsave('figures/SupFig4.png', 
       (SupFig4), bg='transparent',  width = 20, height = 16, dpi = 900, units = "cm", device='png')
...
```{r Supplementary Figure 5: Estimates of power under weak effects, include=FALSE}
SupFig5 <- ggarrange(
...
ggsave('figures/SupFig5.png', SupFig5, device='png', width=20, height=16, units='cm')
```

```{r Supplementary Figure 6: Estimates of power under moderate effects, include=FALSE}
...
ggsave('figures/SupFig6.png', SupFig6, device='png', width=20, height=16, units='cm')
```

### Other figure

The figure below is not included in any texts, however serves as a demonstration. 
Panels a and b compare the correlation of brain measure to behavioural factor in the original sample to the resample. Notice at this stage, that points are duplicated (this is a red herring, or proxy at best). Comparing panels c and d demonstrates that the marginal distribution of edge connectivity is preserved by the resample. This is as described in bootstrapping literature. However, the distribution of resulting brain-wide associations is not preserved (as shown in main text Figure 2).

```{r Suplementary figure ?: Visualising resampling at edge-behaviour level, include=FALSE}
...
resamplingFigure<-ggarrange(os_scatter, rs_scatter, os_edgeHist, rs_edgeHist, nrow=2, ncol=2, labels=c('a','b', 'c', 'd'), heights=c(5,3))
