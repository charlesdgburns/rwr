...
nEffects <- 1225 #100 choose 2; more effects leads to smoother bias estimations
...
nTrue <- 50 #round(nEffects/100); code here for 1% true effects, although only 12 effects lead to noise bias estimates.
...
DiscoveryPlot <- Tibble %>% ggplot(aes(x=power, y=r, colour=real))+ geom_jitter(shape=3, alpha=0.5, show.legend = FALSE) + 
  scale_colour_gradient(low="gray", high='red') + 
  scale_alpha_continuous(range= c(0.1,1))+ 
  geom_hline(yintercept=tanh((qnorm(1-thr/2)/sqrt(nDiscovery-3))), linetype="dashed")+ 
  scale_shape_binned(solid=FALSE) + 
  labs(y= "Observed Effect Size (Pearson r)", x="Statistical Power (Ground truth)", subtitle=paste("Discovery Samples (",nEffects, " effects at N = ",nDiscovery,")", sep="")) +
  theme_classic()
...
Resample <- list()
  ...
  for(p in 1:length(power)){
      for(b in 1:length(binsize)){
        ...
        corrs[,i,b,p] <-  ts/sqrt(n-2+ts^2) #Transform from t statistic to Pearson r.
        
        corrs[,i,b,p] <- corrs[,i,b,p]+Discovery$corrs[,p] #Add the discovery sample effects, as if resampled and varying around observed statistic. Cautionary note: this works only for correlations close to 0 and large sample sizes, as otherwise we might (although rarely) observe correlations exceeding -1 or 1. To avoid this one could sample from the exact distribution of Pearson correlations.
...
palette <- RColorBrewer::brewer.pal(11, "RdYlBu")
  ...
  BiasFigure <- analyticWrangle %>% 
    ggplot() + 
      ...
      scale_x_continuous(breaks=binsize, trans='log10') +
      scale_fill_discrete(labels=thr) + labs(subtitle='Bias (estimated - expected)', y='Statistical Power', x='Sample Size', colour='\U003B1') +
    scale_colour_manual(values=palette, labels=c("0.01"," "," "," "," "," "," "," "," ","0.99"))+ 
    guides(colour = guide_coloursteps(even.steps=TRUE, show.limits=FALSE,direction= 'horizontal',reverse=0, barheight = unit(0.3, "cm"), barwidth=unit(2.1,"cm"),label.position='top', title.position="top", title="Discovery Power"))+
      theme(legend.position=c(0.2,0.35))
